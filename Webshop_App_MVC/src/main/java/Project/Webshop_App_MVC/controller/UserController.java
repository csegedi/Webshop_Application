...
public class UserController {
	...
	//Cart:temporary list for the product
	...
	public String categories(Model model, 
		...
		if ((username != null) && (password != null)) { //RequestParam is used
			...
			for (int customersIndex = 0; customersIndex < customers.size(); customersIndex++) {
				...
				if ((current.getUsername().equals(username)) && (current.getPassword().equals(password))
					...
					//cookies are created by the first login
					...
					//the user's rating list are filled with products
		...
		else if (cookieName != null && cookiePassword != null) { // RequestParam doesn't exist, CookieValues are used
	...
	public String cart(Model model, @RequestParam(required = false, name = "selected_quantity") Integer quantity,
		...
		if (selectedProductId != null) {
			...
			/** ADD PRODUCT TO THE CART */
				...
				if (assistant == 0) { // if the selected product isn't in the cart (new product)/add the product to
										// cart with the incoming quantity
	...
	public String selectProductToRating(Model model, @CookieValue(name = "cookie_Username") String username,
			@CookieValue(name = "cookie_Password") String password) {
	...
	public String addRating(Model model, @RequestParam(required = false, name = "selectedProduct") Integer productId,
